...
class MotionDetector:
    ...
    def clear_background(self, camera_name: str):
        """清理指定摄像头的背景缓存。"""
        self.backgrounds.pop(camera_name, None)
    ...
    def detect(
        ...
        bg = self.backgrounds.get(camera_name)
        if bg is None:
            self.backgrounds[camera_name] = gray.astype(np.float32)
            return [], False

        # 摄像头切流/重连后分辨率可能变化，背景尺寸不一致时重建
        if bg.shape != gray.shape:
            slog.warning(
                f"MotionDetector background shape changed for {camera_name}: "
                f"{bg.shape} -> {gray.shape}, resetting background"
            )
            self.backgrounds[camera_name] = gray.astype(np.float32)
            return [], False
        ...
        try:
            ...
            # 兜底：类型/通道不一致时重置背景，避免任务持续报错
            slog.warning(
                f"MotionDetector accumulateWeighted failed for {camera_name}, "
                f"resetting background: {e}"
            )
            self.backgrounds[camera_name] = gray.astype(np.float32)
            return [], False

        frame_delta = cv2.absdiff(
            gray, cv2.convertScaleAbs(bg)
        )
